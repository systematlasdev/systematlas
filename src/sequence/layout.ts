...
export function buildSequence(doc: SequenceDocument, dims: SeqDims): SeqLayout {
  ...
  const emit = (c: SequenceCall, depth: number) => {
    ...
    callYc[c.id] = yc;
    rows.push({ call: c, depth, y, yc, h: rowH, kind: "call" });
    y += rowH;
    c.children?.forEach((ch) => emit(ch, depth + 1));
    if (hasReturn(c)) {
      const ryc = y + returnH * 0.5;
      returnYc[c.id] = ryc;
      rows.push({ call: c, depth, y, yc: ryc, h: returnH, kind: "return" });
      y += returnH;
    }
  };
  doc.calls.forEach((c) => emit(c, 0));
  ...
  rows.forEach((r) => {
    if (r.kind === "call" && r.call.children?.length) {
      acts.push({ actorIdx: actorIndex[r.call.to] ?? 0, y1: r.yc, y2: lastDescYc(r.call), depth: r.depth });
    }
  });
